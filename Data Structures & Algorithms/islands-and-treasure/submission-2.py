class Solution:
    ...
    def findPathToTreasure(self, i, j,grid):
        ...
        seen = [[0 for col in grid[0]] for row in grid]
        to_traverse = deque()
        ...
        while len(to_traverse) > 0:
            ...
            if grid[a][b] == 0:
                ...
                for (x,y) in directions:
                    new_x, new_y = a+x, b+y
                    if new_x >= 0 and new_x<len(grid) and new_y >= 0 and new_y<len(grid[0]) and seen[new_x][new_y] == 0 and grid[new_x][new_y] != -1 and (new_x,new_y) not in to_traverse:
